...
function attachmentTemplate(fileType, fileName = "", imgURL = null) {
  if (fileType == "save-offer") {
    return (
      `
 <div class="attachment-preview">
     <span class="fas fa-times cancel"></span>
     <button class="offer-btn"><h3><p style="padding:0px 30px;"><span class="fas fa-file"></span> YOUR OFFER </p></h2></button>
 </div>
`
    );
...
function sendMessage() {
  ...
  let offerName = !!$(".offer-name").val();
  const inputValue = $.trim(messageInput.val());
  if (inputValue.length > 0 || hasFile || offerName) {
    console.log("success");
...
$(document).ready(function () {
  ...
  $("body").on("click", ".offer-btn", function (e) {
    e.preventDefault();
    app_modal({
      show: true,
      name: "offer",
  ...
  $("body").on("click", ".open-offer-btn", function (e) {
    e.preventDefault();
    app_modal({
      show: true,
      name: "open-offer",
...
$("body").on("click", ".save-offer-btn", function () {
  var offerName = $("textarea[name='offerName']").val().trim();
  var offerContent = $("textarea[name='offerContent']").val().trim();
  var offerPrice = $("textarea[name='offerPrice']").val().trim();

  if (offerName !== "" && offerContent !== "" && offerPrice !== "") {
    let sendCard = $(".messenger-sendCard");
    sendCard.find(".attachment-preview").remove();
    sendCard.prepend(attachmentTemplate("save-offer"));
  }
});